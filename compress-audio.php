...
if( isset($_REQUEST["path"]) ) { //check path parameter if exist
    ...
    if( isset( $_REQUEST["q"]) ) { //if quality is set, add the set value to the default value
        ...
        $quality_query = "-q:a $quality"; 
    }

    $command = "ffmpeg -y -i $media_path $quality_query ./output/$temp_filename.mp3"; //command for the ffmpeg
   ...
   $response = array(
    'filesize' => filesize("./output/$temp_filename.mp3"),
    'file'=> $temp_filename.".mp3"
    );
